...
from dataset_synapse import Synapse_dataset, RandomGenerator,RandomGenerator_test
...
def inference(args, model, error_name,epoch_num, test_save_path=None):
    db_test_training = Synapse_dataset(base_dir='../data/final/iCTCF_6_24', list_dir=args.list_dir, split=args.val_txt,is_train = False,transform=transforms.Compose(
                                   [RandomGenerator_test(output_size=[args.img_size, args.img_size])]), test_time=True)
    db_test = Synapse_dataset(base_dir='../data/final/iCTCF_6_24', list_dir=args.list_dir, split=args.val_txt,is_train = False)
    ...
    print("The ordinal AUC is:", ordinal_auc_(metrics.confusion_matrix(Y_val_set,result)))
        #print(Y_val_set)
        #print(result_)
